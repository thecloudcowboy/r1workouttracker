...
`}),s+=`
`,s+=`Recent Workouts:
`,e.slice(0,5).forEach(i=>{s+=`- ${b(i.startTime)}: ${i.exercises.length} exercises, ${v(i.duration)}
...
${e}`,wantsJournalEntry:!0})):(console.log("Report:",e),alert("Report exported to console (journal entry in R1 device)"))}function J(e){if(typeof PluginMessageHandler<"u"){const s={message:JSON.stringify({query_params:{engine:"google_images",q:`${e} exercise demonstration`,hl:"en",gl:"us",tbs:"sur:cl",ijn:0,num:1},useLocation:!1}),useSerpAPI:!0};PluginMessageHandler.postMessage(JSON.stringify(s))}}window.onPluginMessage=function(e){if(console.log("Received plugin message:",e),e.data)try{const s=typeof e.data=="string"?JSON.parse(e.data):e.data;if(s.images_results&&s.images_results.length>0){const a=s.images_results[0].thumbnail||s.images_results[0].original;t.selectedExercise&&t.exercises[t.selectedExercise]&&(t.exercises[t.selectedExercise].imageUrl=a,l(),d())}}catch(s){console.error("Error parsing plugin message:",s)}};function d(){const e=document.getElementById("app");switch(t.currentScreen){case"home":U(e);break;case"library":z(e);break;case"workout":Y(e);break;case"history":te(e);break;case"reports":ae(e);break;case"exerciseDetail":Q(e);break;case"addExercise":oe(e);break;case"addCategory":ne(e);break;case"favorites":V(e);break;case"stopwatch":ue(e);break;default:U(e)}}function U(e){e.innerHTML=`
                      ...
                      <div class="set-item">
                        ...
                        <div class="set-inputs">
                          <div class="set-input-row">
                            ...
                            <input type="number" inputmode="decimal" placeholder="0" value="${r.reps||""}" 
                                   data-action="updateSet" data-exercise="${i}" data-set="${c}" data-field="reps" 
                                   autocomplete="off">
                          </div>
                          <div class="set-input-row">
                            <span class="set-input-label">Weight:</span>
                            <input type="number" inputmode="decimal" placeholder="0" value="${r.weight||""}" 
  ...
  `,ee()}function ee(){var e,s,a;(e=document.querySelector('[data-action="cancel"]'))==null||e.addEventListener("click",()=>{confirm("Cancel workout? All progress will be lost.")&&D()}),(s=document.querySelector('[data-action="finish"]'))==null||s.addEventListener("click",()=>{if(t.activeWorkout.exercises.length===0){alert("Add at least one exercise before finishing");return}A()}),(a=document.querySelector('[data-action="addExercise"]'))==null||a.addEventListener("click",()=>{n("library")}),document.querySelectorAll('[data-action="addSet"]').forEach(i=>{i.addEventListener("click",()=>{F(parseInt(i.dataset.exercise))})}),document.querySelectorAll('[data-action="updateSet"]').forEach(i=>{i.addEventListener("input",o=>{const r=parseInt(o.target.dataset.exercise),c=parseInt(o.target.dataset.set),p=o.target.dataset.field,h=parseFloat(o.target.value)||0;T(r,c,p,h)}),i.addEventListener("focus",o=>{o.target.setAttribute("inputmode","decimal")})}),document.querySelectorAll('[data-action="updateExerciseNotes"]').forEach(i=>{i.addEventListener("blur",o=>{const r=parseInt(o.target.dataset.exercise);j(r,o.target.value)})}),document.querySelectorAll('[data-action="removeExercise"]').forEach(i=>{i.addEventListener("click",()=>{const o=parseInt(i.dataset.index);t.activeWorkout.exercises.splice(o,1),l(),u(),d()})}),document.querySelectorAll('[data-action="removeSet"]').forEach(i=>{i.addEventListener("click",()=>{const o=parseInt(i.dataset.exercise),r=parseInt(i.dataset.set);t.activeWorkout.exercises[o].sets.splice(r,1),l(),u(),d()})})}function te(e){const s=Object.values(t.workouts).sort((a,i)=>i.startTime-a.startTime);e.innerHTML=`
            ...
            ${s.map(a=>`
              <div class="history-item" data-workout="${a.id}">
                ...
                <div class="history-info">
                  ...
                  <div class="history-duration">${v(a.duration)}</div>
                </div>
  ...
  `,ce()}function ce(){var e,s;(e=document.querySelector('[data-action="back"]'))==null||e.addEventListener("click",()=>n("addExercise")),(s=document.getElementById("addCategoryForm"))==null||s.addEventListener("submit",a=>{a.preventDefault();const i=new FormData(a.target);I(i.get("name"),i.get("muscleGroup")),n("addExercise")})}function S(){t.stopwatch.isRunning=!0,t.stopwatch.startTime=Date.now()-t.stopwatch.elapsed}function E(){t.stopwatch.isRunning=!1,t.stopwatch.elapsed=Date.now()-t.stopwatch.startTime}function le(){t.stopwatch.isRunning=!1,t.stopwatch.startTime=null,t.stopwatch.elapsed=0,t.stopwatch.laps=[]}function de(){const e=t.stopwatch.isRunning?Date.now()-t.stopwatch.startTime:t.stopwatch.elapsed;t.stopwatch.laps.push(e)}function ue(e){const s=t.stopwatch.isRunning?Date.now()-t.stopwatch.startTime:t.stopwatch.elapsed,a=s%1e3,i=Math.floor(s/1e3),o=i%60,r=Math.floor(i/60)%60,p=`${Math.floor(i/3600).toString().padStart(2,"0")}:${r.toString().padStart(2,"0")}:${o.toString().padStart(2,"0")}.${Math.floor(a/10).toString().padStart(2,"0")}`;e.innerHTML=`
    <div class="screen stopwatch-screen">
      ...
      <div class="stopwatch-display">
        <div class="stopwatch-time">${p}</div>
      ...
      ${t.stopwatch.laps.length>0?`
        <div class="scrollable-content">
          <div class="laps-section">
            <h3>Laps</h3>
            <div class="laps-list">
              ${t.stopwatch.laps.map((h,W)=>{const $=h%1e3,f=Math.floor(h/1e3),L=f%60,P=Math.floor(f/60)%60,B=`${Math.floor(f/3600).toString().padStart(2,"0")}:${P.toString().padStart(2,"0")}:${L.toString().padStart(2,"0")}.${Math.floor($/10).toString().padStart(2,"0")}`;return`
                  <div class="lap-item">
  ...
  `,pe()}function pe(){var e,s,a,i;(e=document.querySelector('[data-action="back"]'))==null||e.addEventListener("click",()=>n("home")),(s=document.querySelector('[data-action="start"]'))==null||s.addEventListener("click",()=>{S(),d()}),(a=document.querySelector('[data-action="stop"]'))==null||a.addEventListener("click",()=>{E(),d()}),document.querySelectorAll('[data-action="reset"]').forEach(o=>{o.addEventListener("click",()=>{le(),d()})}),(i=document.querySelector('[data-action="lap"]'))==null||i.addEventListener("click",()=>{de(),d()})}window.addEventListener("scrollUp",()=>{console.log("Scroll up detected")});window.addEventListener("scrollDown",()=>{console.log("Scroll down detected")});window.addEventListener("sideClick",()=>{if(console.log("Side button clicked"),t.currentScreen==="home"&&!t.activeWorkout)x();else if(t.currentScreen==="workout"){if(t.activeWorkout&&t.activeWorkout.exercises.length>0){const e=t.activeWorkout.exercises.length-1;F(e)}}else t.currentScreen==="stopwatch"&&(t.stopwatch.isRunning?E():S(),d())});document.addEventListener("DOMContentLoaded",async()=>{console.log("Workout Tracker initialized"),typeof PluginMessageHandler>"u"&&window.addEventListener("keydown",e=>{e.code==="Space"&&(e.preventDefault(),window.dispatchEvent(new CustomEvent("sideClick")))}),await R(),M(),d(),setInterval(()=>{if(t.activeWorkout&&t.currentScreen==="workout"){const e=document.querySelector(".workout-timer");if(e){const s=Math.floor((Date.now()-t.activeWorkout.startTime)/1e3);e.textContent=v(s)}}t.stopwatch.isRunning&&t.currentScreen==="stopwatch"&&d()},100),setInterval(async()=>{await l(),t.activeWorkout&&await u()},1e4),window.addEventListener("beforeunload",async()=>{await l(),t.activeWorkout&&await u()}),document.addEventListener("visibilitychange",async()=>{document.hidden&&(await l(),t.activeWorkout&&await u())})});
