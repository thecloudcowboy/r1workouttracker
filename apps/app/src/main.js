...
async function saveToStorage(key, value) {
  try {
    if (window.storageManager) {
      await window.storageManager.setItem(key, value);
    } else {
      console.warn('Storage manager not available, using direct localStorage');
...
async function removeFromStorage(key) {
  try {
    if (window.storageManager) {
      await window.storageManager.removeItem(key);
    } else {
...
async function clearActiveWorkout() {
  try {
    await removeFromStorage('activeWorkout');
